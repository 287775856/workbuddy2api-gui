...
| ➕ **添加账号** | 网页版 OAuth 设备授权：生成授权链接 → 自动轮询 → 自动落盘 → 按配置自动重启网关加载 |
...
| 🔧 **系统** | 网关身份校验（`service=workbuddy2api`）、容器状态与健康检查、面板运行信息、任务历史、客户端接入示例 |
...
| `ui.username` / `ui.password` | `admin` / `workbuddy` | 面板登录凭据 |
...
`WBGUI_LISTEN` · `WBGUI_GATEWAY_URL` · `WBGUI_GATEWAY_API_KEY` · `WBGUI_AUTH_DIR` · `WBGUI_CONFIG_FILE` ·
...
| `POST` | `/api/login/start` | 发起 OAuth 授权，返回 `auth_url` |
...
| `authstore` | 双格式解析、**uid 路径穿越防护**、空 token 拒写、坏文件容错、权限 |
| `api` | 口令校验、**登录防爆破锁定**、会话过期、Origin 同源校验 |
| `upstream` | 错误分类优先级、文案可读性（HTML 错误页净化为中文建议）、幂等判定 |
...
**Q：网页登录报"申请授权失败"？**
面板所在服务器无法访问 `copilot.tencent.com`。可在能出网的地方用 `./login.sh` 登录，
再用「账号管理 → 导入凭证」把 `auths/workbuddy-*.json` 的内容粘贴进来。

**Q：加了账号但账号池里没有？**
网关只在启动时扫描 `auths/`。到「系统」页点「重启网关」，或命令行 `docker restart workbuddy2api`。

**Q：改配置了但没生效？**
同上，`config.json` 只在网关启动时读取，需要重启。
